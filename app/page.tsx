...
const ProtectedPage: FunctionComponent = () => {
  ...
  const [chatList, setChatList] = useState([{ title: "S7b0t4", id: 852493309, }]);
    ...
    axios.get('link')
      .then(function (response) {
        ...
        results.forEach((element: any) => {
          if (element.message) {
            let data: Chat = { title: "S7b0t4", id: 52493309 };
            if (element.message.chat.title) {
              data = { title: element.message.chat.title, id: element.message.chat.id };
            } else if (element.message.chat.username) {
              data = { title: element.message.chat.username, id: element.message.chat.id };
            }

            if (!chatList.some(chat => chat.title === data.title) && !newChats.some(chat => chat.title === data.title)) {
              newChats.push(data);
            }
          }
        });
  ...
  return (
    <div>
      ...
      <div className="flex flex-col w-3/5 m-auto gap-3">
        ...
        <ul>
          {chatList.map(chat => (
            <li key={chat.id} onClick={() => handleChatSelect(chat)} className="cursor-pointer">
              {chat.title}
            </li>
          ))}
        </ul>
